#!/usr/bin/env python3
"""Debug soft-map fold on 1CRN only."""
...
from pairfold.soft_contact_fold import rank_decoy, soft_map_scaffold
...
def main():
    ...
    ca = dihedrals_to_ca(r["phis"], r["psis"])
    print(
        "pipeline rmsd",
        kabsch(ca, native),
        "anchors",
        r["contacts"].get("n_anchors"),
        "note",
        (r.get("notes") or r.get("note") or "")[:200],
    )

    probs = cp.contact_probs(seq, model="primary")
    probs30 = cp.contact_probs(seq, model="alt")
    local = pred._overlap_refine(seq)
    ph, ps = local["phis"], local["psis"]
    print("local rmsd", kabsch(dihedrals_to_ca(ph, ps), native))

    best = None
    for name, pmap in [("t12", probs), ("t30", probs30)]:
        for seed in (5, 19, 37, 51):
            soft = soft_map_scaffold(
                seq, ph, ps, pmap, n_steps=550, fit_steps=1800, seed=seed, contact_thresh=0.10
            )
            ca_s = dihedrals_to_ca(soft["phis_deg"], soft["psis_deg"])
            rms = kabsch(ca_s, native)
            print(
                f"{name} seed{seed} improved={soft['improved']} "
                f"rank {soft['base_rank']:.3f}->{soft['best_rank']:.3f} "
                f"phys {soft.get('phys_cand', float('nan')):.3f} "
                f"sel {soft.get('select_score', float('nan')):.3f} rmsd={rms:.3f}"
            )
            if soft["improved"] and (best is None or soft["select_score"] > best[0]):
                best = (soft["select_score"], rms, f"{name}@{seed}")
    print("selected", best)

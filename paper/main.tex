...
\author{%
  ...
  \texttt{arefimahdi840@gmail.com}\\
  Local Computational Biology Project\\
  \small Code and benchmarks:
  \url{https://github.com/laravel840/PairFold}
}

\date{\today}

% Auto-filled by benchmarks/write_paper_tables.py (placeholders until benches finish)
\input{macros_domains.tex}
\input{macros_long.tex}
...
\begin{table}[t]
  ...
  \label{tab:class}
  \input{table_class_means.tex}
\end{table}

\begin{figure}[t]
  \centering
  \IfFileExists{benchmark_plots_expanded.png}{%
    \includegraphics[width=\textwidth]{benchmark_plots_expanded.png}%
  }{%
    \fbox{\parbox{0.92\textwidth}{\centering\vspace{2em}Figure pending: run \texttt{plot\_expanded.py} after Domains100.\vspace{2em}}}%
  }
...
\begin{figure}[t]
  \centering
  \IfFileExists{benchmark_ablation_plot.png}{%
    \includegraphics[width=0.72\textwidth]{benchmark_ablation_plot.png}%
  }{%
    \fbox{\parbox{0.72\textwidth}{\centering\vspace{1.5em}Ablation figure pending.\vspace{1.5em}}}%
  }
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    \input{table_method_compare.tex}
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \IfFileExists{benchmark_long100_time.png}{%
    \includegraphics[width=0.72\textwidth]{benchmark_long100_time.png}%
  }{%
    \fbox{\parbox{0.72\textwidth}{\centering\vspace{1.5em}Long-chain timing figure pending.\vspace{1.5em}}}%
  }
...
Source code, checkpoints used for inference, expanded benchmark CSVs, and this manuscript are available at
\url{https://github.com/laravel840/PairFold}.
All software components described herein (\texttt{pairfold/predict.py}, \texttt{pairfold/clash\_assembly.py}, \texttt{pairfold/structure\_blocks.py}, and the \texttt{benchmarks/} suite) are part of that repository.
